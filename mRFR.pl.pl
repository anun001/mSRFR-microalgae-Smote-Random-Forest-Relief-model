...
system("java -classpath \$CLASSPATH:weka.jar:libsvm.jar:mysql-connector-java-5.0.8-bin.jar weka.classifiers.trees.RandomForest -l mRFR.model -T $filename.new.csv -p 0  > $filename.out");
...
print("\n\n########## Prediction results by mRFR/smRFR: A machine learning model using microalgal signature features for ncRNA classification ##########\n\n");

system("grep 1 $filename.out | awk '{gsub(/+/,\" \")}; {print \"  Sequence no. \"\$1 \" predict as \" \$3  \" - with a probability of \"\$4 \n}' |head -n -1 ");
